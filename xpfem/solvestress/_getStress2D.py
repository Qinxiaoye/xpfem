...
class getStress2D:
    ...
    def evualStress(self, displacement, region):
        ...
        for n in range(sumElem):
            ...
            Bu = np.dot(Bmat[12 * n:12 * (n + 1), :], elemU)
            for m in range(4):
        ...
        for n in range(sumElem):
            ...
            stress[elem[n, :], :] += np.dot(invs,s[4 * n:4 * (n + 1), :])
    ...
    def evualMassB(self, region):
        ...
        self.gauss_int = math.gauss_legendre(intOrder=2, dim=2)
        nge = len(self.gauss_int.points) * 3
    ...
    def evualB2D(self, x, y):
        ...
        elemB = np.zeros((len(self.gauss_int.points) * 3, self.mnode * 2))
        for i in range(len(self.gauss_int.points)):
            ks, yt = self.gauss_int.points[i]
