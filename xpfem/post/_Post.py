...
class Post:
    ...
    def showDisplacement(self,region,displacement,direction = 'all',sym = 'none',showEdge=True,
                        show_deformed = False, scale = 1.0, show_shade = False,opacity = 0.2,smooth = True,div = 9):
        # 显示位移，二维和三维
        sumElem = region.mesh.sumElem
        node = region.mesh.points
        elem = region.mesh.cells
        ...
        if smooth:
            dargs = dict(
                ...
                interpolate_before_map=not smooth,
            )
        else:
            dargs = dict(
                cmap="coolwarm",
    ...
    def showStress(self,region,displacement,stress,mises,direction = 'sxx',sym = 'none',showEdge=True,
        ...
        if self.ndim == 2:
            ...
            map = {'sxx':0,'syy':1,'szz':2,"sxy":3,"syx":3,'sxz':4,'szx':4,'syz':5,'szy':5}
